...
interface MoveAllNotebookQuestionsModalProps {
  folders: QuizFolder[];
...
export const MoveAllNotebookQuestionsModal: React.FC<MoveAllNotebookQuestionsModalProps> = ({ folders, currentFolderId, currentNotebookId, onConfirm, onClose }) => {
      ...
      <div className="bg-[#0A0F1E] w-full max-w-md rounded-[50px] shadow-2xl border border-white/10 overflow-hidden animate-in zoom-in-95 slide-in-from-bottom-10 duration-500">
        <div className="p-10">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-2xl font-black italic tracking-tighter text-white">MOVER TODAS AS QUESTÕES</h3>
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] block ml-2">Escolha a Pasta Destino</label>
              <select
              ...
              >
                <option value="" className="bg-[#0A0F1E]">Selecione uma pasta</option>
                {folders.filter(f => f.id !== currentFolderId).map(f => <option key={f.id} value={f.id} className="bg-[#0A0F1E]">{f.name}</option>)}
              </select>
            </div>

            {selectedFolder && (
              <div className="space-y-4">
                <label className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] block ml-2">Escolha o Caderno Destino</label>
